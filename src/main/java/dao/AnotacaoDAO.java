...
public class AnotacaoDAO {
    ...
    public boolean inserirAnotacao(Anotacao Anotacao) {
        ...
        try {
            ...
            st.executeUpdate("INSERT INTO Anotacao (Psicologo_ID, Anotacao_ID, Nome, Motivo, Criacao, Obs)" + "VALUES ("
                    + Anotacao.getPsicologoId() + ", '" + Anotacao.getAnotacaoId() + "', '"
    ...
    public boolean excluirAnotacao(int id) {
        ...
        try {
            Statement st = conexao.createStatement();
            st.executeUpdate("DELETE FROM Anotacao WHERE Anotacao_ID = " + id);
            st.close();
            status = true;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                for (int i = 0; rs.next(); i++) {
                    Anotacoes[i] = new Anotacao(rs.getInt("Psicologo"), rs.getInt("Anotacao_ID"), rs.getString("Nome_anotacao"),
    ...
    public Anotacao procurarAnotacao(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                Anotacao = new Anotacao(rs.getInt("Psicologo"), rs.getInt("Anotacao_ID"), rs.getString("Nome_anotacao"),
                rs.getString("Motivo"), rs.getDate("Craicao"), rs.getString("Obs"));
            }
